...
void runNetwork() { //runs the program
    ...
    cout<<"Enter 1 to use a file with a prebuilt network. Enter anything else to start from scratch:"<<endl;
    char fileEntry;
    cin>>fileEntry;
    ...
    if (fileEntry == '1'){
        ...
        cin>>inputFileName;
        if(N.readUsers(inputFileName)==0){ //call network function to read in users
            cout<<endl<<"network built!"<<endl;
        }
        id += N.getSize();

        cout<<"Enter 1 to use a file with prebuilt posts, enter anything else for no prebuilt posts:"<<endl;
        char postEntry;
        cin>>postEntry;

        if(postEntry == '1'){ //if the user wants to add prebuilt posts file
            string postFileName;
            cout<<"enter the name of your file:"<<endl;
            cin>>postFileName; //get the filename
            if(N.readPosts(postFileName)==0){ //call network function to read in posts
                cout<<endl<<"Posts Added!"<<endl;
            }
        }
        else{
            cout<<endl<<"no prebuilt posts!"<<endl;
        }
    }
    else{
